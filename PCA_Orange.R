...
pca.churn=prcomp(CV_data, scale=TRUE)

summary(pca.churn)

windows()
plot(pca.churn, type='l')

pca_churn_summary=summary(pca.churn)
plot(pca_churn_summary$importance[3,],
     xlab="Principal Component",
     ylab="Cumulative Proportion of Variance Explained")

windows()
biplot(pca.churn, expand=2, xlim=c(-0.2, 0.2),
       ylim=c(-0.15, 0.15), cex=1.1)
